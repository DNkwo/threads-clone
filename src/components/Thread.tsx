...
const Thread = ({ user, thread, setOpenPopUp, getThreads, setInteractingThread}: Props) => {
  const timePassed = moment().startOf("day").to(thread.timestamp);

  const handleClick = () => {
    setOpenPopUp(true);
    setInteractingThread(thread);
  };

  const postLike = async () => {
    console.log(thread);
    const hasBeenLikedByUser = thread.likes.some(
